...
func GetFilesName() {
	files, err := ioutil.ReadDir("./node_jsons_reverse")
	if err != nil {
		log.Fatal(err)
	}
	...
	for i := 0; i < len(k); i++ {
		byteValue, err := ioutil.ReadFile(k[i])
		if err != nil {
...
func outputjson(filename string, result map[string]string) {
	...
	// Write back to file
	err = ioutil.WriteFile("./nodes_states/"+filename, byteValue, 0644)
	// fmt.Println(file.Name()[:10])
}
...
func mytest() {
	...
	m5["e"] = "on"
	m5["f"] = "on"
	RecordState(globalstate, m5)

	m6 := make(map[string]string)
	m6["b"] = "on"
	m6["c"] = "on"
	RecordState(globalstate, m6)

	m7 := make(map[string]string)
	m7["a"] = "on"
	m7["b"] = "on"
	m7["c"] = "on"
	RecordState(globalstate, m7)
